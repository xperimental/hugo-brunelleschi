<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
				...
				<?php if ( ! empty( $post->post_parent ) ) : ?>
					<p class="page-title"><a href="<?php echo get_permalink( $post->post_parent ); ?>" title="<?php esc_attr( printf( __( 'Return to %s', 'brunelleschi' ), get_the_title( $post->post_parent ) ) ); ?>" rel="gallery"><?php
						printf( __( '<span class="meta-nav">&larr;</span> %s', 'brunelleschi' ), get_the_title( $post->post_parent ) );
					?></a></p>
				<?php endif; ?>
...
?>
						<p class="attachment"><a href="<?php echo $next_attachment_url; ?>" title="<?php echo esc_attr( get_the_title() ); ?>" rel="attachment"><?php
							$attachment_width  = apply_filters( 'brunelleschi_attachment_size', 900 );
							$attachment_height = apply_filters( 'brunelleschi_attachment_height', 900 );
...
<?php endif; ?>
						...
						<div class="entry-caption"><?php if ( !empty( $post->post_excerpt ) ) if( brunelleschi_options('archive-format') ){ the_content(); }else{ the_excerpt(); } ?></div>
